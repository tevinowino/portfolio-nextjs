...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`${geistSans.variable} ${geistMono.variable}`}>
      <Head>
        ...
        <meta name="keywords" content="Tevin Owino, portfolio, web development, React, Next.js, Developers,  JavaScript" />
  ...
  <div className="flex items-center justify-between px-8 py-5">
    {/* Logo */}
    <Link href="/" className="font-bold text-2xl text-white hover:text-blue-400 transition-all duration-500 relative group">
      <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 animate-gradient-x font-extrabold">Tevin Owino</span>
    ...
    <div className="hidden md:flex items-center gap-8">
      {navLinks.map(({ href, icon: Icon, label, special }) => (
        ...
        >
          <Icon size={20} className="group-hover:scale-125 transition-transform duration-500" />
          <span className="font-medium">{label}</span>
          {!special && <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-gradient-to-r from-blue-400 to-purple-500 transition-all duration-500 group-hover:w-full"></span>}
        </Link>
      ))}
      <a
        href="/resume.pdf"
      ...
      >
        <Download size={20} className="group-hover:scale-125 transition-transform duration-500" />
    ...
    >
      {isMenuOpen ? <X size={28} className="transform hover:rotate-90 transition-transform duration-500" /> : <Menu size={28} className="transform hover:scale-110 transition-transform duration-500" />}
    </button>
  </div>
  ...
  <div
    className={`md:hidden absolute top-full left-0 w-full bg-blue-950/90 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl shadow-blue-500/20 transition-all duration-500 ease-in-out ${
      isMenuOpen ? 'opacity-100 translate-y-2' : 'opacity-0 -translate-y-4 pointer-events-none'
    }`}
  >
    <div className="px-6 py-4 flex flex-col gap-4">
      {navLinks.map(({ href, icon: Icon, label, special }) => (
        ...
        >
          <Icon size={22} className="group-hover:scale-125 transition-transform duration-500" />
          <span className="font-medium">{label}</span>
        </Link>
      ))}
      <a
      ...
      >
        <Download size={22} className="group-hover:scale-125 transition-transform duration-500" />
